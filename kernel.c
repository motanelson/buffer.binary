...
void draw_cube();
extern void _switch_to_real_modes();
...
void rects(int x,int y, int xx,int yy , int colors)
{
    hline(x,y,x+xx,(char)colors);
    vline(x+xx,y,y+yy,(char)colors);
    hline(x,y+yy,x+xx,(char)colors);
    vline(x,y,y+yy,(char)colors);


}
void winds(int x,int y, int xx,int yy , int colors)
{
    box(x,y,xx,yy,(char)colors);
    rects(x,y,xx,yy,(char)0);
}        
void kernel_main()
{
		Bitmap *bmp;
		...
		while(t)
                {   
                    for(n=0;n<6;n++)winds(n*12,n*12,100,100,14);
                    sleeps(1);
                    n=getKeys();
                    if(n==1)t=0;
                    cls(0);
